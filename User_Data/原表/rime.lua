...
--  select_character_processor = require("select_chatacter") !!模塊化編程 未叶
--  time_translator = require("time_translator")
function test(input, seg) 
	
	if (input == "iku") then
		local textFile = io.open("C:\\Users\\lenovo\\Desktop\\rimeLog.txt", "a")
		textFile:write("select_character")
		textFile:close()
...
-- -- 1
-- --    local ks= KeySequence() [[創建 KeySequence() 對象
-- --    ks:parse("abc") [[把字符串abc轉成KeySequence對象
-- -- 2
