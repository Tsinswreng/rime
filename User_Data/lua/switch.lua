...
local function select_index(key, env)
  local ch = key.keycode --keycode	number	按键值
  local index = -1
  ...
  if select_keys ~= nil and select_keys ~= "" and not key.ctrl() and ch >= 0x20 and ch < 0x7f then
    local pos = string.find(select_keys, string.char(ch)) --在字符串 select_keys 中查找字符 ch 第一次出现的位置，并返回该位置的索引值。
...
local function apply_switch(env, keyword, target_state)
  local ctx = env.engine.context -- 输入编码上下文。
  local swt = env.switcher -- env：lua table 对象。预设 engine 和 name_space 两个成员，分别是 Engine 对象和前述 name_space 配置字符串。
