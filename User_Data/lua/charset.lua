...
local function exists(single_filter, text)
  for i in utf8.codes(text) do -- utf8.codes(text) 函数返回一个迭代器，用于遍历指定 UTF-8 编码的字符串 text 中的所有 Unicode 码点（code point）。
     local c = utf8.codepoint(text, i) -- ，utf8.codepoint(text, i) 函数返回指定 UTF-8 编码的字符串 text 中第 i 个字符（即第 i 个 Unicode 码点）的整数值。如果 i 参数超出了 text 的范围，则该函数会返回 nil。
